...
public final class GeoCode {
    // latitude and longitude bounds according to Web Mercator projection limits
    ...
    public static boolean isValidCoordinates(double longitude, double latitude) {
        return (GeoCode.MIN_LONGITUDE <= longitude && longitude <= GeoCode.MAX_LONGITUDE) &&
            (GeoCode.MIN_LATITUDE <= latitude && latitude <= GeoCode.MAX_LATITUDE);
    }
    ...
    public static double[] decode(double code) {
        long geoCode = (long) code;
